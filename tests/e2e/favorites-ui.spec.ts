...
test("SC-201: страница избранного показывает пустое состояние", async ({
  ...
  await expect(
    page.getByText("Добавьте записи в избранное, чтобы они появились здесь"),
  ).toBeVisible();
...
test("SC-202: добавление записи через демо-кнопку сердца", async ({ page }) => {
  await page.goto("/");
...
test("SC-203: удаление записи через кнопку X на карточке", async ({ page }) => {
  await page.goto("/");
